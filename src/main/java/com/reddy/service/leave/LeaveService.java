package com.reddy.service.leave;
...
public class LeaveService {
    ...
    private void validateLeaveApplication(Employee employee, LeaveType leaveType, LeaveRequestDTO dto) {
        ...
        List<LeaveRequest> overlapping = leaveRequestRepo
                .findByEmployeeEmpIdAndStatus(employee.getEmpId(), LeaveStatus.APPROVED)
                .stream()
        ...
        LeaveBalance balance = leaveBalanceRepo.findByEmployeeEmpIdAndLeaveTypeId(employee.getEmpId(), leaveType.getId())
    ...
    @Transactional
    public LeaveRequestResponseDTO approveLeave(Long requestId, Long approverId) {
    ...
    private void updateLeaveBalance(LeaveRequest request) {
        LeaveBalance balance = leaveBalanceRepo
                .findByEmployeeEmpIdAndLeaveTypeId(request.getEmployee().getEmpId(), request.getLeaveType().getId())
                .orElseThrow(() -> new LeaveBalanceNotFoundException("Leave balance not found"));
    ...
    private boolean isValidApprover(Employee approver, Employee requester) {
        ...
        return approver.getRole() != null && "MANAGER".equalsIgnoreCase(approver.getRole().getName())
    ...
    public List<LeaveBalance> getEmployeeLeaveBalances(Long employeeId) {
        return leaveBalanceRepo.findByEmployeeEmpId(employeeId);
    }
    ...
    public Page<LeaveRequest> getPendingLeaveRequests(Long departmentId, Pageable pageable) {
        return leaveRequestRepo.findPendingByDepartment(departmentId, pageable);
    }
...
//    private void updateLeaveBalance(LeaveRequest request) {
//        LeaveBalance balance = leaveBalanceRepo
//                .findByEmployeeEmpIdAndLeaveTypeId(
...
//        int daysTaken = calculateWorkingDays(request.getStartDate(), request.getEndDate());
//        balance.setUsedDays(balance.getUsedDays() + daysTaken);
//        leaveBalanceRepo.save(balance); // Automatic update
//    }

//    public Page<LeaveRequestResponseDTO> getPendingLeaveRequests(Long departmentId, Pageable pageable) {
//        return leaveRequestRepo.findPendingByDepartment(departmentId, pageable)
//                .map(this::convertToDTO);
//    }
//    LeaveRequestResponseDTO convertToDTO(LeaveRequest leaveRequest) {
//        LeaveRequestResponseDTO dto = madelMapper.map(leaveRequest, LeaveRequestResponseDTO.class);
//        return dto;
//    };
}
